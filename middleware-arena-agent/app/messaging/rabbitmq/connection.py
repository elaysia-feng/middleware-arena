"""RabbitMQ robust connection and topology declaration."""
...
class RabbitMQManager:
    ...
    async def _declare_topology(self) -> None:
        ...
        self.analysis_exchange = await self.channel.declare_exchange(
            EXCHANGE_ANALYSIS, ExchangeType.DIRECT, durable=True
        )
        self.status_exchange = await self.channel.declare_exchange(
            EXCHANGE_STATUS, ExchangeType.DIRECT, durable=True
        )
        dlx = await self.channel.declare_exchange(
            DLX_ANALYSIS, ExchangeType.DIRECT, durable=True
        )
