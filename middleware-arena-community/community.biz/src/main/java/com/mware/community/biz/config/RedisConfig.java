 ...
 * key 用字符串、value 用 JSON，幂等 key（comment:list:p1:p1:s20）可读，value 可跨语言存取。
 * <p>
 ...
 * 把可实例化类型白名单限定为本项目 dto / JDK 基础类型。
 * <p>
 * 复用 {@code order.config.redis.RedisConfig} 的写法，只把白名单域换成 {@code com.mware.community.dto.}
 */
@Configuration
public class RedisConfig {
    ...
    private ObjectMapper typedObjectMapper() {
        BasicPolymorphicTypeValidator ptv = BasicPolymorphicTypeValidator.builder()
                ...
                .allowIfSubType("java.time.")              // LocalDateTime（CommentResponse.createdAt）
                .allowIfSubType("com.mware.community.dto.") // CommentResponse / PostResponse ...
                .build();

        ObjectMapper mapper = new ObjectMapper();
        // CommentResponse 含 LocalDateTime 字段，不注册 JSR310 模块序列化会抛 InvalidDefinitionException
